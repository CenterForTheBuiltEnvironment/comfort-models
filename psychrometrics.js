...
module.exports = {
    ...
    convert: function(x, tdb, origin, target) {
        switch (origin) {
            case 'rh':
                a = this.tdb_rh(tdb, x);
                break;
    ...
    tdb_w: function(tdb, w) {
        var a = {};
        var psat = this.satpress(tdb);
    
        a.w = w;
        a.rh = 100 * this.relhum(this.PROP.Patm, psat, w);
    ...
    tdb_dewpoint: function(tdb, dewpoint) {
        var w_l = 0.00001,
            w_r = 0.2,
            eps = 0.0001;
        var fn = function(w) {
            var dpstar = this.dewpoint(w);
            return dewpoint - dpstar;
        }
        var w = util.bisect(w_l, w_r, fn, eps, 0);
        return this.tdb_w(tdb, w);
    },
    
    tdb_vappress: function(tdb, vappress) {
        var psat = this.satpress(tdb);
        var rh = 100 * vappress / psat;
    
        return this.tdb_rh(tdb, rh);
    },
    
    drybulb: function(h, w) {
        var PROP = this.PROP;
        return (H - PROP.Hfg * w) / (PROP.CpAir + PROP.CpVap * w);
    },
    
    wetbulb: function(tdb, w) {
        var self = this;
        ...
        fn = function(t) {
            psatStar = self.satpress(t);
            wStar = self.humratio(PROP.Patm, psatStar);
    ...
    relhum: function(patm, psat, humRatio) {
        var pw, rh;
    ...
    sattemp: function(p) {
        ...
        var tsat_l = 0,
            ...
            psat;
        var fn = function(t) {
            return (p - self.satpress(t));
        }
    
        return util.bisect(tsat_l, tsat_r, fn, eps, 0);
    },
    
    tairsat: function(hsat) {
        var tsat_l = 0,
            tsat_r = 1000,
            eps = 0.01;
        var fn = function(t) {
            return (hsat - this.enthsat(t));
        }
        return util.bisect(tsat_l, tsat_r, fn, eps, 0);
    },
    
    globetemp: function(ta, vel, tglobe, diameter, emissivity) {
        pow = Math.pow;
